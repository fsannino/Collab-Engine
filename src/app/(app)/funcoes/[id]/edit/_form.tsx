...
import { updateFuncaoAction } from '@/modules/people/people.actions';

type Props = {
  funcao: { id: string; nome: string; descricao: string | null };
};

export default function FuncaoEditForm({ funcao }: Props) {
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const res = await updateFuncaoAction(funcao.id, data);
      if (res.ok) {
        router.push(`/funcoes/${funcao.id}`);
      } else {
        setError(res.error);
      }
    });
  ...
  return (
    <div style={{ padding: '40px 48px', fontFamily: 'system-ui,-apple-system,sans-serif', maxWidth: '640px' }}>
      <div style={{ marginBottom: '8px' }}>
        <a href={`/funcoes/${funcao.id}`} style={{ color: '#64748b', fontSize: '13px', textDecoration: 'none' }}>← Voltar</a>
      </div>
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '32px' }}>
        <div style={{ width: '4px', height: '24px', background: '#c9a227', borderRadius: '2px' }} />
        <h1 style={{ fontSize: '22px', fontWeight: 700, color: '#0f2244', margin: 0 }}>Editar Função</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
        ...
        <div>
          ...
          <input name="nome" required defaultValue={funcao.nome} style={inputStyle} />
        </div>

        <div>
          <label style={labelStyle}>Descrição <span style={{ fontWeight: 400, color: '#9ca3af' }}>(opcional)</span></label>
          <textarea name="descricao" rows={3} defaultValue={funcao.descricao ?? ''} style={{ ...inputStyle, resize: 'vertical' }} />
        </div>
