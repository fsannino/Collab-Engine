...
function NewPlanFormInner({ projects }: { projects: Project[] }) {
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      ...
      if (res.ok) {
        router.push(`/projects/${projectId}/training`);
      } else {
        setError(res.error);
      }
    });
  ...
  function handleGenerate() {
    if (!projectId) { setError('Selecione um projeto primeiro.'); return; }
    startGenerate(async () => {
  ...
  return (
    <div className="p-6 max-w-lg mx-auto space-y-6">
      ...
      <div>
        ...
        {projects.length === 0 && (
          <p className="text-xs text-amber-600 mt-1">Nenhum projeto cadastrado. Crie um projeto antes de adicionar planos de treinamento.</p>
        )}
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex gap-3 pt-2">
          <button
            ...
            className="px-4 py-2 bg-slate-800 text-white text-sm rounded-lg hover:bg-slate-700 disabled:opacity-50 disabled:cursor-not-allowed"
          >
...
export function NewPlanForm({ projects }: { projects: Project[] }) {
  return (
    <Suspense>
      <NewPlanFormInner projects={projects} />
    </Suspense>
  );
}
