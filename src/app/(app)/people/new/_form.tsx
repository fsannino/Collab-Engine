...
export default function PessoaForm({ areas, pessoas }: { areas: Area[]; pessoas: Pessoa[] }) {
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      ...
      if (res.ok) router.push(`/people/${res.data.id}`);
  ...
  return (
    <div style={{ padding: '40px 48px', fontFamily: 'system-ui,-apple-system,sans-serif', maxWidth: '640px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '32px' }}>
        ...
        <h1 style={{ fontSize: '22px', fontWeight: 700, color: '#0f2244', margin: 0 }}>Nova Pessoa</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
          ...
          <div>
            ...
            <input name="cpf" maxLength={11} pattern="\d{11}" style={inputStyle} />
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
          ...
          <div>
            ...
            <select name="superiorId" style={{ ...inputStyle, background: '#fff' }}>
        ...
        <div>
          ...
          <input name="localidadeTrabalho" style={inputStyle} />
        </div>

        <div>
          <label style={labelStyle}>ID no HRIS <span style={hintStyle}>(opcional)</span></label>
          <input name="hrisId" style={inputStyle} />
        </div>
