...
export const createStakeholderSchema = z.object({
  ...
  organizationLevel: z.nativeEnum(StakeholderLevel, { errorMap: () => ({ message: 'Nível organizacional inválido' }) }).optional(),
  position:          z.nativeEnum(StakeholderPosition, { errorMap: () => ({ message: 'Posição inválida' }) }),
